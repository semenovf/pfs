...
class device_notifier_pool
{
    ...
    typedef ContigousContainer<int>         index_vec_type;

public:
    device_notifier_pool (size_t initial_size = 0)
    {
        if (initial_size > 0) {
            pollfd_type emptyfd = { -1, 0, 0 };
            
            _pollfds.resize(initial_size, emptyfd);
            _pollfds_empty_items.resize(initial_size);
            
            for (int i = initial_size - 1; i >= 0; i--)
                _pollfds_empty_items[i] = i;
        }
    }
...
protected:
#endif
    void insert (int fd, short notify_events)
    {
        short poll_events = 0;
        ...
        // Get empty item if available
        if (_pollfds_empty_items.size() > 0) {
            int index = _pollfds_empty_items.back();
            _pollfds_empty_items.pop_back();
        } else {
            pollfd_type emptyfd = { -1, 0, 0 };
            index = integral_cast_check<int>(_pollfds.size());
            _pollfds.resize(_pollfds.size() * 2, emptyfd);
        }
...
private:
    ...
    index_vec_type  _pollfds_empty_items;
};
