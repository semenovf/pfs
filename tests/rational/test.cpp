...
int main ()
{
    ...
    std::cout << "///////////////////////////////////////////////////////////////////////////\n";
    TEST_OK(to_string(rational())         == pfs::string("0/1"));
    TEST_OK(to_string(rational(1))        == pfs::string("1/1"));
    TEST_OK(to_string(rational(-1))       == pfs::string("-1/1"));
    TEST_OK(to_string(rational(1, 1))     == pfs::string("1/1"));
    TEST_OK(to_string(rational(-1, 1))    == pfs::string("-1/1"));
    TEST_OK(to_string(rational(1, -1))    == pfs::string("-1/1"));
    TEST_OK(to_string(rational(-1, -1))   == pfs::string("1/1"));
    TEST_OK(to_string(rational(2, 1))     == pfs::string("2/1"));
    TEST_OK(to_string(rational(7, 1))     == pfs::string("7/1"));
    TEST_OK(to_string(rational(4, 2))     == pfs::string("4/2"));
    TEST_OK(to_string(rational(7, 10))    == pfs::string("7/10"));
    TEST_OK(to_string(rational(7, 100))   == pfs::string("7/100"));
    TEST_OK(to_string(rational(7, 1000))  == pfs::string("7/1000"));
    TEST_OK(to_string(rational(7, 10000)) == pfs::string("7/10000"));
    TEST_OK(to_string(rational(7, 11))    == pfs::string("7/11"));

    TEST_OK(to_string(rational(pfs::ratio<1, 1>()))     == pfs::string("1/1"));
    TEST_OK(to_string(rational(pfs::ratio<-1, 1>()))    == pfs::string("-1/1"));
    TEST_OK(to_string(rational(pfs::ratio<1, -1>()))    == pfs::string("-1/1"));
    TEST_OK(to_string(rational(pfs::ratio<-1, -1>()))   == pfs::string("1/1"));
    TEST_OK(to_string(rational(pfs::ratio<2, 1>()))     == pfs::string("2/1"));
    TEST_OK(to_string(rational(pfs::ratio<7, 1>()))     == pfs::string("7/1"));
    TEST_OK(to_string(rational(pfs::ratio<4, 2>()))     == pfs::string("2/1"));
    TEST_OK(to_string(rational(pfs::ratio<7, 10>()))    == pfs::string("7/10"));
    TEST_OK(to_string(rational(pfs::ratio<7, 100>()))   == pfs::string("7/100"));
    TEST_OK(to_string(rational(pfs::ratio<7, 1000>()))  == pfs::string("7/1000"));
    TEST_OK(to_string(rational(pfs::ratio<7, 10000>())) == pfs::string("7/10000"));
    ...
    std::cout << "//                                 Reduce                                //\n";
    std::cout << "///////////////////////////////////////////////////////////////////////////\n";
    TEST_OK(pfs::to_string(rational( 1, 2).reduce())  == pfs::to_string("1/2"));
    TEST_OK(pfs::to_string(rational( 2, 4).reduce())  == pfs::to_string("1/2"));
    TEST_OK(pfs::to_string(rational(-8, 16).reduce()) == pfs::to_string("-1/2"));

    std::cout << "///////////////////////////////////////////////////////////////////////////\n";
    std::cout << "//                                Invert                                 //\n";
    std::cout << "///////////////////////////////////////////////////////////////////////////\n";
    TEST_OK(pfs::to_string(rational( 1, 2).invert())  == pfs::string("2/1"));
    TEST_OK(pfs::to_string(rational( 4, 2).invert())  == pfs::string("2/4"));
    TEST_OK(pfs::to_string(rational(-8, 16).invert()) == pfs::string("-16/8"));
